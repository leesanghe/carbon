...
<body>
	...
	<form name="search"
		action="${pageContext.request.contextPath}/actboard" method="post">
		<input type="hidden" name="curPage" value="${searchVO.curPage }">
		<input type="hidden" name="rowSizePerPage"
			value="${searchVO.rowSizePerPage }">
		<!-- 추가된 부분 -->
		<input type="hidden" name="totalPageCount"
			value="${searchVO.totalPageCount }"> <input type="hidden"
			name="firstPage" value="${searchVO.firstPage }"> <input
			type="hidden" name="lastPage" value="${searchVO.lastPage }">
		<!-- 추가된 부분 끝 -->
		<div class="row g-0 justify-content-md-center mt-10">
			<div class="col-sm-1 mb-3 ml-4">

				<select id="searchType" name="searchOption"
					class="form-control input-sm">
					<option value="T"
						${searchVO.searchOption eq "T" ? "selected='selected'":"" }>제목</option>
					<option value="W"
						${searchVO.searchOption eq "W" ? "selected='selected'":"" }>작성자</option>
					<option value="C"
						${searchVO.searchOption eq "C" ? "selected='selected'":"" }>내용</option>
				</select>
			...
			<div class="col-sm-5 mb-3">
				<div class="input-group mb-4">
					<input class="form-control" name="keyword" placeholder="검색어를 입력하세요"
						aria-label="search" value="${searchVO.keyword}" type="search">
			...
			<tbody>
				<c:forEach items="${actboardList }" var="act">
					<tr>
						<td>${act.boardNo }</td>
						<td><a
							href="<c:url value="/actdetail?boardNo=${act.boardNo }"/>">${act.boardTitle }</a></td>
						<td><a
							href="<c:url value="/actdetail?boardNo=${act.boardNo }"/>">${act.memId }
						</a></td>
