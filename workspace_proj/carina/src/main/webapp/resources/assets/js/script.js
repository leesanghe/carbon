const questions = [    {        question: "1. 나냠 냠 .",        options: {            A: "O",            B: "X"        }    },    {        question: "2. 나는 일을 계획적으로 처리하는 편이다.",        options: {            A: "그렇다",            B: "아니다"        }    },    {        question: "3. 나는 어려운 문제를 해결하는 것을 좋아한다.",        options: {            A: "그렇다",            B: "아니다"        }    },    {        question: "4. 나는 항상 새로운 아이디어를 생각해내는 편이다.",        options: {            A: "그렇다",            B: "아니다"        }    },    {        question: "5. 나는 규칙을 따르는 것보다 독자적인 생각을 추구하는 편이다.",        options: {            A: "그렇다",            B: "아니다"        }    }];
...
const showResult = (mbtiType) => {
    const resultfirst = document.querySelector("#result");
    resultfirst.classList.remove("d-none");
    const mbtiTypefirst = document.querySelector("#mbti-type");
    const recommendedJobfirst = document.querySelector("#recommended-job");
    mbtiTypefirst.innerText = mbtiType.type;
    recommendedJobfirst.innerText = mbtiType.job;
};
...
const restartTest = () => {
    ...
    answers = {};
    const resultfirst = document.querySelector("#result");
    resultfirst.classList.add("d-none");
    showQuestion();
};
    ...
    $("#question").on("click", "#next-btn", () => {
        ...
        if (answer) {
            ...
            if (currentQuestion < questions.length) {
                ...
                let mbtiType = "";
                    if (typeCounts > typeCounts.E) mbtiType += "I";
                    else mbtiType += "E";
