...
public class MemberController {
	...
	public String loginView(HttpServletRequest request, HttpSession session, Model model, String msg) {
	    // 현재 페이지의 URL을 세션에 저장
	...
	public String loginDo(MemberVO member, HttpSession session,
	                      boolean remember, String fromUrl, HttpServletResponse response, Model model) {
...
//	        return "redirect:/"; // 로그인 성공 시 이동할 페이지
	        String fromUrl1 = (String) session.getAttribute("fromUrl");
	       
	        if (!fromUrl1.startsWith("http://192.168.0.40:8080/loginView") ) {
	        	System.out.println(fromUrl);
	            session.removeAttribute("fromUrl"); // 이후에는 세션에서 삭제
	            return "redirect:" + fromUrl1;
	        } else {
	            // 이전 페이지가 없는 경우 기본적으로 리다이렉션할 페이지 설정
	            return "redirect:/";
	        }
