...
//		$('div.step1').hide();
//		$('div.step3').show();
//		$('div#content div.page2 div.pageDiv').css('background' , "url('/tanso2018/images/page2_bg2.png') no-repeat");
		...
		$('li[class^="pageMenu_level"]').click(function(){
			...
			if($(this).prop('class').indexOf('on') == -1){
				...
				$(this).addClass('on'); // 클릭시 자기 자신 온 표시
				nowLevel = thisLevel;
				
				if(nowLevel == 1){
					
					$('.subTitleDetail').html("월간 사용량(권장)과 요금 중 <b>한 가지만 입력</b>");
					$('.subTitleName').html("<i class='sub-icon01'></i> 전기");
					$('.subTitleDetail2').html("요금 입력 시 실제 사용량과 차이가 발생할 수 있습니다.");
					$('.imgbox span').attr('class','imgbox-img1');
					...
					$('.subTitleDetail').html("월간 사용량(권장)과 요금 중 <b>한 가지만 입력</b>");
					$('.subTitleName').html("<i class='sub-icon02'></i> 가스");
					$('.subTitleDetail2').html("요금 입력 시 실제 사용량과 차이가 발생할 수 있습니다.");
					$('.imgbox span').attr('class','imgbox-img2');
					...
					$('.subTitleDetail').html("월간 사용량(권장)과 요금 중 <b>한 가지만 입력</b>");
					$('.subTitleName').html("<i class='sub-icon03'></i> 수도");
					$('.subTitleDetail2').html("요금 입력 시 실제 사용량과 차이가 발생할 수 있습니다.");
					$('.imgbox span').attr('class','imgbox-img3');
					...
					$('.subTitleDetail').html("이동거리(권장)와 연료비 중 <b>한 가지만 입력</b>");
					$('.subTitleName').html("<i class='sub-icon04'></i> 교통");
					$('.subTitleDetail2').html("연료비 입력 시 실제 사용량과 차이가 발생할 수 있습니다.");
					$('.imgbox span').attr('class','imgbox-img4');
		...
		// 2022-12-21 기존스크립트 참고용 나중에 제거 bk
		/*
		$('li[class^="pageMenu_level"]').click(function(){
			...
			if($(this).find('img').prop('src').indexOf('on') == -1){
				...
				if(nowLevel == 1){
					...
					$('div.subTitle div.subTitleDetail').html("ㆍ월간 사용량(권장)과 요금 중 <b>한가지만 입력</b>");
					$('div.subTitleName').html("전기");
					$('div.subTitleName').addClass("subTitleName1");
					...
					$('div.subTitleName').addClass("subTitleName2");
					$('div.subTitleDetail2').html("ㆍ요금 입력 시 실제 사용량과 차이가 발생할 수 있습니다.");
					...
					$('div.subTitleName').addClass("subTitleName3");
					$('div.subTitleDetail2').html("ㆍ요금 입력 시 실제 사용량과 차이가 발생할 수 있습니다.");
					...
					$('div.subTitle div.subTitleDetail').html("ㆍ이동거리(권장)와 연료비 중 <b>한가지만 입력</b>");
					$('div.subTitleName').html("교통");
					$('div.subTitleName').addClass("subTitleName4");
		...
		*/
		...
		$('input[name="usedElec"]').bind('keyup' , function(){
			...
			if(price == 0){
				...
				elecCo2 =  Math.round(( price  * 0.4781 ) / 0.1) * 0.1;
				printNumber('co2' , calc(elecCo2));
				
				if(elecCo2 == 0){
					elecTree = 0;
				}else{
					elecTree = Math.round( ((price * 0.4781) / 4.13) / 0.1) * 0.1;
				}
		...
		// 2022-12-16 새로운 계산식 추가
		$('input[name="moneyElec"]').bind('keyup' , function(){
			
			$('input[name="usedElec"]').val("");
			$(this).val( ($(this).val()).replace(/[^0-9]/g,'') );

			var elec_hr = $('input[name="elec_highrow"]:checked').val();	//고압 H, 저압 R
			var elec_se = $('input[name="elec_month"]:checked').val();		//하계 S, 기타 E
			var euType = '1';
			if(elec_hr == 'H' && elec_se == 'S'){
				euType = '1';
			} else if(elec_hr == 'R' && elec_se == 'S'){
				euType = '2';
			} else if(elec_hr == 'H' && elec_se == 'E'){
				euType = '3';
			} else {
				euType = '4';
			}
			...
			if(price == 0){
				...
				var imsiKwh = 0;	//사용량
				$.ajax({
					url:'http://localhost:8080/cbs/energyCalculator/calculateCar',
					data:{'price':price, 'euType':euType},
					success:function(data){
						
						if(data != "0" || data != "" || data != null){
							imsiKwh = data.euUsed;
							elecCo2 =  Math.round((imsiKwh  * 0.4781)/0.1)*0.1;
							printNumber('co2' , calc(elecCo2));
							elecTree = Math.round(((imsiKwh * 0.4781) / 4.13)/0.1)*0.1;
							printNumber('tree' , calc(elecTree));
							total();
						} else {
							elecCo2 = 0;
		...
		$('input[name="elec_highrow"], input[name="elec_month"]').click(function(){
			
			var price = price==''?0:parseInt($('input[name="moneyElec"]').val().replace(/[^0-9]/g,''));   // 전기요금
	
			if(price > 0){ // 전기요금에 값이 있을 경우만
		...
		*/
		...
		$('input[name="usedGas"]').bind('keyup' , function(){
			...
			if(price == 0){
				...
				imsiCo2 =  Math.round((price * 2.176) / 0.1) * 0.1;
				...
				if(imsiCo2 == 0){
					...
					gasTree = Math.round( ( gasCo2 / 4.13) /0.1) * 0.1;
					printNumber('tree' , calc(gasTree));
		...
		$('input[name="moneyGas"]').bind('keyup' , function(){
			...
			if(price == 0){
				...
				if(price > 0 && price <= 412){
					...
					imsiCo2 =  Math.round((price - 1100)  / 799.92); // 824.378 => 799.92
				}
				...
				if(imsiCo2 == 0){
					...
					gasCo2 = calc(Math.round((imsiCo2 * 2.176) / 0.1) * 0.1);
					printNumber('co2' , calc(gasCo2));
		...
		$('input[name="usedWater"]').bind('keyup' , function(){
			...
			if(price == 0){
				...
				waterCo2 = calc(Math.round( (price * 0.237) / 0.1) * 0.1);
				...
				if(waterCo2 == 0){
					...
					waterTree =  Math.round( ( waterCo2 / 4.13) / 0.1 ) * 0.1;
		...
		$('input[name="moneyWater"]').bind('keyup' , function(){
			...
			if(price == 0){
				...
				if(price >= 1 && price <= 480){ // 2023-01-10 330 => 480 이하일 경우 무조건 1 bk
					imsiWater = 1;
				}else{
					imsiWater = Math.round(price / 719); // 660.4 => 719
				}
				
				waterCo2 = calc(Math.round( (imsiWater * 0.237) / 0.1) * 0.1);
				...
				if(waterCo2 == 0){
					...
					waterTree =  Math.round( ( waterCo2 / 4.13) / 0.1 ) * 0.1; // 12.4 => 4.13
					printNumber('co2' , calc(waterCo2));
					printNumber('tree' , calc(waterTree ));
		...
		$('input[name="usedCar"]').bind('keyup' , function(){
			...
			if(oilType == 3){
				...
				if(oilType == 0){	//휘발유
					oilCo2 =  calc(Math.round(((price / 16.04) * 2.097) / 0.1) * 0.1);			
				}else if(oilType == 1){ //경유
					oilCo2 =  calc(Math.round(((price / 15.35) * 2.582) / 0.1) * 0.1);
				}else if(oilType == 2){ //LPG
					oilCo2 =  calc(Math.round(((price / 11.06) * 1.868) / 0.1) * 0.1);
				}
				...
				if(oilCo2 == 0){
					...
					oilTree = calc(Math.round( ( oilCo2 / 4.13) / 0.1) * 0.1 );
		...
		$('input[name="moneyCar"]').bind('keyup' , function(){
			...
			if(oilType == 3){
				...
				if(oilType == 0){	//휘발유
					oilCo2 =  calc(Math.round(((price / 1590.6) * 2.097) / 0.1) * 0.1);	
				}else if(oilType == 1){ //경유
					oilCo2 = calc(Math.round(((price / 1391.4 ) * 2.582) / 0.1) * 0.1);
				}else{	//LPG
					oilCo2 = calc(Math.round( ( (price / 932.24 ) * 1.868) / 0.1) * 0.1);
				}
		...
		$('.saveBtn').click(function(){
			...
			if($.trim($('input[name="memberName"]').val()) == ""){
				...
				var targetUrl = "http://localhost:8080/cbs/tanso/insert_data.green";
				
				if(resultIdx != 0){	//update
					targetUrl = "http://localhost:8080/cbs/tanso/update_data.green";
...
//						oilCo2 = 177;
						...
						var overCnt = 0;
						console.log('co2_elec_avg:', co2_elec_avg);
						console.log('elecCo2:', elecCo2);
						if(co2_elec_avg < elecCo2) overCnt++;
						console.log('co2_gas_avg:', co2_gas_avg);
						console.log('gasCo2:', gasCo2);
						if(co2_gas_avg < gasCo2) overCnt++;
						console.log('co2_water_avg:', co2_water_avg);
						console.log('waterCo2:', waterCo2);
						if(co2_water_avg < waterCo2) overCnt++;
						console.log('co2_car_avg:', co2_car_avg);
						console.log('oilCo2:', oilCo2);
						if(co2_car_avg < oilCo2) overCnt++;
						console.log('overCnt:', overCnt);
						$('.overCount').html(overCnt);
						
						avgTotalCo2Percent=1000;
...
//				$('#topHeader ul li:eq(1)').click();
				...
				if($('input[name="usedElec"]').val() =='' &&  $('input[name="moneyElec"]').val() == '' ){
					...
					$('.pageMenu_level3').click();
				}else if( $('input[name="oilCar"]:checked').val() != '3' &&  ( $('input[name="usedCar"]').val() =='' &&  $('input[name="moneyCar"]').val() == ''   ) ){
					alert('이동 거리 혹은 연료비를 입력해 주시기 바랍니다.');
				}else{
					inputok = true;
					$('#topHeader ul li:eq(1)').click();
				}
		...
		$('#topHeader ul li').click(function(){
			
			if(!$(this).hasClass('on')){
				
				var idx = $('#topHeader ul li').index(this);
				
				if(idx == 1 && inputok == false){

				}else{
					$('#topHeader ul li.on').removeClass('on');
					$(this).addClass('on');
			...
			if(!$(this).hasClass('on')){
				...
				if(idx == 1){	//사용량분석
					...
					if(insertOk == true){
						...
						var co2_total_tree_new = calc(Math.round( (co2_total_value / 4.13) / 0.1 )* 0.1); 
...
//					var car_avg_total = calc(co2_car_avg + oilCo2 + target_car_co2);
		...
		$('div.page2 div.step3 ul.menu li').click(function(){
			if(!$(this).hasClass('on')){
				...
				//$('div.page2 div.step3 div.right div.result_sale').css('background' , "url('/tanso2018/images/step2_right_bg"+ (idx +1) +".jpg') no-repeat");
				//$('div.page2 div.step3 div.right div.result_bottom').css('background' , "url('/tanso2018/images/step2_right_bottom_"+ (idx +1) +".jpg') no-repeat");
				...
				$('.page2 .step3 .left .result_middle').css('background' , bgColor);
				$('.page2 .step3 .left').css('background' , bgColor);
		...
		$('.page3').find('.tapMenu ul li').click(function(){
			...
			$('div.pageDiv').find('.contentDiv .contList').hide();
			$('div.pageDiv').find('.contentDiv .contList:eq('+index+')').show();
		});
...
//		var avg = calc(val1 + val2 + val3);
		...
		var val1_cm = calc(250 *val1_percent);
		if(val1_cm > 150) val1_cm = 150;
		...
		var val2_cm = calc(250 *val2_percent);
		if(val2_cm > 150) val2_cm = 150;
		...
		var val3_cm = calc(250 *val3_percent);
		if(val3_cm > 150) val3_cm = 150;
	...
	function showStep3(){
		...
		var saleTotalTree = Math.floor( (saleTotalCo2 * 0.4781) / 4.13);
	...
	function saveData(type){
		...
		if(type == 'down'){
			...
			$.ajax({
			    url: 'http://localhost:8080/cbs/tanso/save_sale.green',
			    type: 'POST',
			    data: JSON.stringify({
			...
			$.ajax({
				...
				success : function(data){
					...
					if(data.msg == "ok"){
						...
						$.ajax({
							url : 'http://localhost:8080/cbs/tanso/sendMail.green',
							type : 'POST',
							data : {
								'memberIdx'		:	resultIdx,
