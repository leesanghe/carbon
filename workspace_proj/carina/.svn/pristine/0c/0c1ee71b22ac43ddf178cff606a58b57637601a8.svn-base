...
<div class="container mt-7 text-center">
    ...
    <a class="btn-3d blue" href="<c:url value='/solution'/>" id="moveToOtherPage">솔루션 알아보기</a>
</div>
	    ...
	    $(document).ready(function() {
	        ...
	        function updatePredictionInfo(year, metro, city, averagePowerUsage, selectedPercentage) {
	            ...
	            var predictionInfoText = year + "년 " + metro + " " + city + " 평균 전력 예측 사용량은 " + averagePowerUsage + "입니다.<br>" +
	                ...
	                "<br><button onclick='CarbonCalculate()' class='btn btn-primary' style='margin-top:40px; margin-right:40px'>탄소포인트 계산알아보러가기</button>" +
	                "<button onclick='CarbonInformation()'class='btn btn-primary' style='margin-top:40px'>탄소포인트알아보러가기</button>";
	...
	<script>
		...
		function CarbonCalculate(){
		    window.open("https://cpoint.or.kr/user/info/experience.do", "_blank");


		}
		
		function CarbonInformation(){
		    window.open("https://cpoint.or.kr/user/guide/cpoint.do", "_blank");
